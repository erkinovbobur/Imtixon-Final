...
const Lipstick = () => {
  ...
  if (isError) return <div className="text-center text-red-500">Error fetching lipstick data</div>;

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-center mb-8 text-white">Lipstick Products</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {data?.slice(95  ).map((product) => (
          <div
            key={product.id}
            className="bg-white rounded-lg shadow-lg p-4 flex flex-col justify-between h-full"
          >
<img
  src={product.image_link || "https://avatars.mds.yandex.net/i?id=efef34e4213be4e60a86fd2ad3add196_l-5299794-images-thumbs&n=13"}
  alt={product.name}
