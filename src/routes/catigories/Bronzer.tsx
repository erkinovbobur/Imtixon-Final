...
import { useBronzerQuery } from "../../redux/api/usersApi";
...
const Bronzer = () => {
  const { data, isLoading, isError } = useBronzerQuery();
  ...
  if (isError) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-red-500 text-xl">Error fetching bronzer data. Please try again later.</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-center mb-8 text-white">Bronzer Products</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {data?.slice(30).map((product: Product, index: number) => (
          <div
            key={product.id}
