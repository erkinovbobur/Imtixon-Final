...
import { useEyelinerQuery } from "../../redux/api/usersApi";
...
const Eyeliner = () => {
  const { data, isLoading, isError } = useEyelinerQuery();
  ...
  const handleLikeToggle = (productId: number) => {
    ...
    if (likedProducts.some((likedProduct: number) => likedProduct === productId)) {
  ...
  if (isError) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-red-500 text-xl">Error fetching eyeliner data. Please try again later.</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-center mb-8 text-white">Eyeliner Products</h1>
          ...
          >
            ...
            <div className="absolute top-3 right-3 p-[15px]">
              <div className="relative">
                ...
                >
                 {likedProducts.some((likedProduct: number ) => likedProduct === product.id) ?  <FcLike /> : <AiOutlineHeart />  }
                </button>
              </div>
            </div>
